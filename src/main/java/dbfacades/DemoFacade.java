...
public class DemoFacade {
  ...
  You can use the file: persistence_TEMPLATE.xml (in this folder) as your template
    ...
    public Car getCarsByMake(String make) {
        EntityManager em = emf.createEntityManager();
        try {
            return (Car) em.createQuery("select m from Car as m where m.make = :make").setParameter("make", make).getSingleResult();
        } finally {
            em.close();
        }
    }
    ...
    public Car getCarById(int id) {
        EntityManager em = emf.createEntityManager();
        try {
            return (Car) em.createQuery("select m from Car as m where m.id = :id").setParameter("id", id).getSingleResult();
        } finally {
            em.close();
        }
    }

    public void getDeleteCarById(Integer id) {
          EntityManager em = emf.createEntityManager();
        try {
            em.getTransaction().begin();
